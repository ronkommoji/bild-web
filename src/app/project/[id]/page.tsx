      ...
      <header className="border-b border-[#E8DCC8] bg-[#FFFDF1] px-6 py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-[#562F00]">{project.name}</h1>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-[#E8DCC8] bg-white p-5">
            <p className="text-sm font-medium text-[#A08050]">Completion</p>
          ...
          <div className="rounded-xl border border-[#E8DCC8] bg-white p-5">
            <p className="text-sm font-medium text-[#A08050]">Active tasks</p>
            <p className="mt-1 text-2xl font-bold text-[#562F00]">{pending.length}</p>
          </div>
        </div>

        <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-xl font-bold text-[#562F00]">Task pipeline</h2>
          <div className="flex gap-2">
            <Link
              href={`/project/${id}/reports`}
              className="rounded-lg border border-[#E8DCC8] px-4 py-2 font-medium text-[#562F00] hover:bg-[#FFF5EB]"
            >
              Reports
            </Link>
            <Link
              href={`/project/${id}/task/new`}
              className="rounded-lg bg-[#FF9644] px-4 py-2 font-medium text-white hover:bg-[#e0853a]"
            >
              + Add task
            </Link>
          </div>
            ...
            <tbody>
              {sorted.map((task) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/project/${id}/task/${task.id}#proof`}
